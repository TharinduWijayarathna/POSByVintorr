        ...
        <tbody>
            <tr>
                <td style="padding-left: 0;">
                    <div class="text-head" style="margin-bottom: 10px; color: {{ $config->color_code }}"><b>SALARY SLIP
                            - {{ $payroll->code }} </b></div>
                </td>
            </tr>
            <tr>
                <td align="left" class="td-customer-style " style="vertical-align:text-top;padding-left: 0;">
                    ...
                    <div style="margin-bottom: 4px;margin-top: 4px;">
                        {{ $payroll->supplier ? $payroll->supplier_name : 'Walking Supplier' }}</div>
                    @isset($payroll->supplier->company)
                    <div style="margin-bottom: 4px;margin-top: 4px;">{{ $payroll->supplier->company }}</div>
                    @endisset
                    @isset($payroll->supplier->address)
                    <div style="margin-bottom: 4px;margin-top: 4px;">{{ $payroll->supplier->address }}</div>
                    @endisset
                    @isset($payroll->supplier->contact)
                    <div style="margin-bottom: 4px;margin-top: 4px;">{{ $payroll->supplier->contact }}</div>
                    @endisset
                    @isset($payroll->supplier->email)
                    <div style="margin-bottom: 4px;margin-top: 4px;">{{ $payroll->supplier->email }}</div>
                    @endisset
                ...
                <td align="right" width="13%" class="td-customer-style "
                    ...
                    <div> {{ $payroll->code }} </div>
                    <div style="margin-bottom: 3px;margin-top: 3px;">
                        {{ \Carbon\Carbon::parse($payroll->created_at)->format('d/m/Y') }}</div>
                </td>
            </tr>
        </tbody>
    </table>
...
<table cellspacing="0" cellpadding="0" border="0" width="100%" class="invoice_table">
    ...
    <tbody>
        <tr class="row-bg">
            <td align="left" class="td-style" style="vertical-align: bottom; font-size:11; white-space: pre-line;">
                @if (isset($payroll->description))
                {{ $payroll->description }}
                @else
                No description available
                @endif
            </td>
            <td align="right" class="td-style right-text" style="vertical-align: bottom; font-size:10;">
...
{{-- <div class="remark-note"><span style="font-size:11px; margin-top:10px; opacity: 0.5;">*This is a computer generated voucher. No
