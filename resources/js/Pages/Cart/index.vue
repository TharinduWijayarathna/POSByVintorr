<template>
    <AppLayout title="POS Cart">
        <template #content>
            <div ref="fullscreenContent" id="full-screen" class="bg-white">
...
orderProduct, key
        ...
        <template #modal>
                                    ...
                                    <div class="col-2 col-md-2">
                                        <div class="py-0 row h-100">
                                            <div class="col-12 justify-content-end text-end">
                                                <a class="py-5 cursor-pointer square-customer-button" @click.prevent="searchLoyaltyCustomer
                                                " data-toggle="tooltip" data-placement="bottom"
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            <a href="javascript:void(0)"
                                                ...
                                                :class="{
                                                    active: tabIndex === 1,
                                                }" data-toggle="tooltip" data-placement="bottom" title="Amount" @click="
                                                    (tabIndex = 1),
                                                    (percentage = '')
                                                    " data-kt-button="true">
            ...
            <!-- QR scanner modal -->
            <div class="modal fade" id="qrScannerModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
...
const searchByName = async (name, product_category_id) => {
    try {
        ...
        // Only auto-select if search term exactly matches a barcode
        const exactBarcodeMatch = products.value.find(product =>
            product.barcode && product.barcode === name // Exact match only, no case conversion
        );

        if (exactBarcodeMatch) {
            // If exact barcode match found, select that product
            await selectProduct(exactBarcodeMatch.id);
        }
        // Do not auto-select if no exact barcode match
        loading.value = false;
    } catch (error) {
        loading.value = false;
    }
...
const toggleFullScreen = () => {
    document.getElementById("full-screen").style.padding = "20px";
