<template>
    <AppLayout title="Tax Report">
        <template #content>
                        ...
                        <div
                            ...
                            <div class="col-12 col-sm-6 d-none d-md-flex justify-content-end">
                                <a @click.prevent="printReport" href="javascript:void(0)" class="btn btn-info fw-bold"
                                    data-toggle="tooltip" data-placement="bottom" title="Print tax report">
                                    <i class="bi bi-printer"></i>
                                    PRINT
                                </a>
                                <a @click.prevent="exportReport" href="javascript:void(0)" data-toggle="tooltip"
                                    data-placement="bottom" title="Export tax report" class="btn btn-info ms-4 fw-bold">
                            ...
                            <div class="p-3 text-sm card-body p-md-9">
                                ...
                                <div class="mb-3 row align-items-center d-none d-lg-flex">
                                    <div class="mb-2 col-xl-3 col-xxl-2 mb-xl-0 pe-0">
                                        <label for="status" class="pt-0 pb-1 text-gray-600 col-form-label">TYPE</label>
                                        <Multiselect v-model="select_order_type" :options="order_types"
                                ...
                                <div class="px-2 py-3 text-sm filters-margin card-body d-block d-lg-none">
                                    <div class="d-flex justify-content-between align-items-end">
                                        <div class="space-x-4">
                                            <button class="p-5 square-clear-button" @click.prevent="openTaxFilterModal">
                                        ...
                                        <table class="table mt-5 align-middle table-hover table-striped fs-6 gy-3">
                                            <thead>
                                                <tr class="text-white text-start fw-bold fs-7 text-uppercase"
                                                    ...
                                                    <th class="table-align-max">CUSTOMER</th>
                                                    <th class="table-align-max">DATE</th>
                                                    <th class="text-center table-align-max">CURRENCY</th>
                                                    <th class="text-end table-align-min">Total Without Tax</th>
                                                    <th class="text-end table-align-max">Total With Tax</th>
                                                    <th class="pe-3 text-end table-align-max">Total Tax</th>
                                                </tr>
                                            </thead>
                                            <tbody class="text-gray-600 fw-semibold">
                                                <tr v-for="order in orders" :key="order.id" class="cursor-pointer">
                                                    ...
                                                    <td @click="handleOrderClick(order)" class="py-2 table-align-max">{{
                                                        order.customer_name ?? 'Walking Customer' }}</td>
                                                    <td @click="handleOrderClick(order)" class="py-2 table-align-max">{{
                                                    ...
                                                    <td @click="handleOrderClick(order)"
                                                        class="py-2 text-end pe-10 table-align-max">{{
                                                            order.formatted_sub_total }}</td>
                                                    <td @click="handleOrderClick(order)"
                                                        class="py-2 text-end pe-3 table-align-max">{{
                                                            numberFormatter(order.total) }}</td>
                                                    <td @click="handleOrderClick(order)"
                                                        class="py-2 text-end pe-3 table-align-max">{{
                                                            numberFormatter(order.total_tax) }}</td>
                                                </tr>
                                                <tr v-if="select_currency != null && select_currency.id != 0"
                                                    class="mb-5 total-row">
                                                    <td class="py-3 ps-3 table-align-max"></td>
                                                    <td class="py-3 table-align-max"></td>
                                                    <td class="py-3 table-align-max"></td>
                                                    <td class="py-3 table-align-max"></td>
                                                    <td class="py-3 pe-10 table-align-max"></td>
                                                    <td
                                                        class="py-3 pt-4 text-gray-600 text-end pe-3 table-align-max fs-4 ps-0 fw-bold">
                                                        Total
                                                    </td>
                                                    <td
                                                        class="py-3 pt-4 text-gray-600 fs-4 ps-0 text-end pe-3 table-align-max fw-bold">
                                                        {{ select_currency ? select_currency.code : "" }} {{
                                                            numberFormatter(total_tax_sum) }}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                                ...
                                                <tr class="odd" v-for="order in orders" :key="order.id">
                                                    <td class="table-responsive-width" @click="handleOrderClick(order)">
                                                        ...
                                                        <div class="row row-divider ">
                                                            ...
                                                            <div class="col-7 col-sm-8 right-side text-end">
                                                                <span>{{
                                                                    numberFormatter(order.customer_name) }}</span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="row row-divider ">
                                                            <div
                                                                class="text-gray-400 col-5 col-sm-4 left-side text-uppercase">
                                                                TOTAL WITHOUT TAX</div>
                                                            <div class="col-7 col-sm-8 right-side text-end ">
                                                                <span>{{
                                                                    numberFormatter(order.sub_total) }}</span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="row row-divider ">
                                                            ...
                                                            <div class="col-7 col-sm-8 right-side text-end ">
                                                                <span>{{
                                                                    numberFormatter(order.total_tax)
                                                                }}</span>
                                                            </div>
                                                        </div>
                                    ...
                                    <table class="table mt-5 align-middle table-hover table-striped fs-6 gy-3">
                                        <tbody class="text-gray-600 fw-semibold">
                                            <tr class="text-gray-700 align-top fw-bold">
                                                ...
                                                <td class="py-3 table-align-max"></td>
                                                <td class="py-3 pe-10 table-align-max"></td>
                                                <td class="py-3 text-end pe-3 table-align-max fs-4 ps-0">Total Tax</td>
                                                <td class="py-3 fs-4 ps-0 text-end pe-3 table-align-max">
                                                    {{ select_currency ? select_currency.code : "" }} {{
                                                        numberFormatter(total_tax_sum) }}</td>
                                            </tr>
                                        ...
                                        <tbody>
                                            <tr class="">
                                                <td class="table-responsive-width">
                                                    ...
                                                    <div class="row row-divider ">
                                                        <div
                                                            class="p-5 col-5 col-sm-4 text-uppercase fs-4">
                                                            TOTAL TAX</div>
                                                        <div class="p-5 col-7 col-sm-8 text-end fs-4 ">
                                                            <span> {{ select_currency ? select_currency.code : "" }} {{
                                                                numberFormatter(total_tax_sum) }}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
            ...
            <!-- Tax Filter Modal -->
            <div class="modal fade" id="taxFIlterModal" data-backdrop="static" tabindex="-1" role="dialog"
                aria-labelledby="taxFIlterModal" aria-hidden="true">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="items-center text-muted">
                                        <label for="modalStatus" class="text-gray-600 col-form-label">TYPE</label>
                                        <Multiselect id="modalStatus" v-model="select_order_type" :options="order_types"
                                ...
                                <div class="col-6">
                                    <div class="py-4 mt-2">
                                        <button @click="taxFIlterModalClear" class="btn btn-info ms-4 fw-bold">
                                ...
                                <div class="col-6">
                                    <div class="py-4 mt-2 text-right">
                                        <button @click="applyTaxFilter" class="btn btn-info ms-4 fw-bold">
...
const to_date = ref(new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0));
...
const total_tax_sum = ref(0);

const select_order_type = ref({ name: 'All', id: 0 });
...
const openTaxFilterModal = () => {
    loading_bar.value.start();
    $('#taxFIlterModal').modal('show');
    loading_bar.value.finish();
};

const applyTaxFilter = () => {
    $('#taxFIlterModal').modal('hide');
    reload();
};

const taxFIlterModalClear = async () => {
    search.value = null;
    select_search_customer.value = null;
    from_date.value = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
    to_date.value = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0);
...
const reload = async () => {
    ...
    const tableData = (
        await axios.get(route('report.tax.all'), {
            params: {
                page: page.value,
                ...
                search_order_type: search_order.value.order_type_id,
            },
        })
    ).data;
    ...
    total_tax_sum.value = tableData.total_tax_sum;
...
const exportReport = async () => {
    try {
        ...
        const fileName = `TaxReport-${formattedDate}.xlsx`;
...
const clearFilters = async () => {
    ...
    to_date.value = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0);
    select_currency.value = null;
    select_order_type.value = { name: 'All', id: 0 };
    search_order.value = {
        order_type_id: null,
        from_date: from_date.value,
        to_date: to_date.value,
...
//     if (newValue) {
//         search_order.value.order_type_id = newValue.id;
//     } else {
//         search_order.value.order_type_id = null;
//     }
//     getSearch();
// });
...
watch(select_order_type, (newValue) => {
    if (newValue) {
        search_order.value.order_type_id = newValue.id;
    } else {
        search_order.value.order_type_id = null;
