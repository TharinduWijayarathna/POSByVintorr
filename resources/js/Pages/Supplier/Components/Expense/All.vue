<template>
    <div class="row" v-if="emptyImageVal == 1">
        <div class="text-center col-12 mt-15">
            <img src="@/../src/images/EmptyState/expenses.webp" height="200" />
        </div>
        <div class="mt-8 text-center col-12">
            <h1 class="text-gray-700 heading fw-bold fs-2hx">No any expenses</h1>
        </div>
        <div class="mt-2 text-center col-12 mb-15">
            <p class="text-gray-700 fs-3">Expenses will appear here</p>
        </div>
    </div>
    <div v-else class="page-count-show page-count-supplier-mobile">
        <div v-if="expenses.length > 0" class="pb-6 col-12 justify-content-end align-self-end d-none d-md-flex">
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5 mt-md-5">
                ...
                <tbody class="text-gray-600 fw-semibold">
                    <tr v-for="expense in expenses" class="cursor-pointer">
                        ...
                        <td class="py-2 text-center ps-3" @click.prevent="viewExpense(expense.id)">{{
                            expense.currency_name }}</td>
                        <td class="py-2 text-end pe-3" @click.prevent="viewExpense(expense.id)">{{
                            expense.formatted_amount }}</td>
                        <td class="py-2 text-end pe-1">
                            <a href="javascript:void(0)" class="p-2" @click.prevent="expensePrint(expense.id)"
    ...
    <!-- Pagination -->
    <div v-if="expenses.length > 0" class="my-3 row">
...
const getSupplierExpenses = async () => {
    nextTick(() => {
        loading_bar.value.start();
    });
    const tableData = (
        await axios.get(route('supplier.expenses.all', props.supplierId), {
            params: {
                page: page.value,
