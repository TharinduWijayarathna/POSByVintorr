                    ...
                    <div class="mt-5 col-lg-12 mt-lg-0">
                        ...
                        <div
                            ...
                            <h1 class="return-main-header">
                                Return&nbsp;<span class="text-gray-600 fs-1">-&nbsp;{{
                                    returnData.code }}</span>
                            </h1>
                            <template v-if="returnData.deleted_at == null">
                                <!-- Add Customer Button -->
                                <button v-if="selectCustomer == null" type="button" data-toggle="tooltip"
                                    data-placement="bottom" title="Add customer" class="btn btn-info btn-sm fw-bold"
                                    @click="showCustomerModal">
                                    <i class="bi bi-plus-lg fw-bold"></i> Add Customer
                                </button>

                                <!-- Remove Customer Button -->
                                <button v-if="selectCustomer != null" type="button" @click.prevent="removeCustomer"
                                    data-toggle="tooltip" data-placement="bottom" title="Remove customer"
                                    class="btn btn-info btn-sm fw-bold">
                                    <i class="bi bi-x-lg fw-bold"></i> {{ truncateCustomerText(selectCustomerName) }}
                                </button>
                        ...
                        <!-- Mobile View -->
                        <div class="mt-1 row d-flex d-sm-none col-form-label">
                            <div class="col-12">
                                <h1 class="return-main-header">
                                    Return&nbsp;<span class="text-gray-600 fs-1">-&nbsp;{{
                                        returnData.code }}</span>
                            ...
                            <div class="text-right col-12">
                                <template v-if="returnData.deleted_at == null">
                                    <a v-if="selectCustomer == null" href="#" data-toggle="tooltip"
                        ...
                        <div class="shadow card">
                            <div class="p-4 text-sm card-body p-md-9">
                                    ...
                                    <form id="addNewCardValidation" class="mt-0 row w-100 pe-0 pe-lg-3"
                                        ...
                                        <div class="col-lg-3 pe-0 pe-lg-3">
                                            ...
                                            <input type="number" class="form-control form-control-solid form-control-sm"
                                                ...
                                                placeholder="0.00" :disabled="returnData.deleted_at != null" />
                                        </div>
                                        <div class="col-lg-3 pe-0 pe-lg-3">
                                            ...
                                            <input type="number" class="form-control form-control-solid form-control-sm"
                                                ...
                                                :disabled="returnData.deleted_at != null" />
                                        </div>
                                        <div
                                            class="mt-3 text-right col-lg-2 mt-lg-0 d-md-block d-lg-flex align-self-end align-item-right pe-0">
                                            <button v-if="returnData.deleted_at == null" type="submit"
                                                data-toggle="tooltip" data-placement="bottom" title="Add item"
                                                class="btn btn-info btn-sm fw-bold"><i
                                ...
                                <div class="row table-responsive gx-0">
                                    <div class="table">
                                        <table
                                            class="table mt-5 align-middle table-responsive table-hover table-striped fs-6 gy-3 mt-md-10"
                                            ...
                                            <thead>
                                                <tr class="text-white text-start fw-bold fs-7 text-uppercase"
                                                    ...
                                                    <th v-if="returnData.deleted_at == null"></th>
                                                </tr>
                                            </thead>
                                            <tbody class="text-gray-600 fw-semibold">
                                                ...
                                                <tr v-for="(returnItem, index) in returnProducts">
                                                    ...
                                                    <td v-if="returnData.deleted_at == null" class="py-2 text-end pe-3">
                                ...
                                <div class="mt-5 mb-2 row">
                                    <div class="col-lg-9 col-xxl-10">
                                        <textarea v-model="remark" class="form-control" placeholder="Remark"
                                            ...
                                            :disabled="returnData.deleted_at != null"></textarea>
                                        <small v-if="validationErrors.remark"
                                            class="mt-1 text-sm text-danger form-text text-error-msg error">
                                            {{ validationErrors.remark }}
                                        </small>
                                    </div>
                                    <div class="mt-3 col-lg-3 col-xxl-2 mt-lg-0">
                                        <div v-if="returnData.deleted_at == null" data-toggle="tooltip"
                                            data-placement="bottom" title="Update return">
                                            <a href="javascript:void(0)" @click.prevent="returnUpdate"
                                        ...
                                        <div v-else data-toggle="tooltip" data-placement="bottom"
                                            title="Restore return">
                                            <a href="javascript:void(0)" @click.prevent="restoreReturn"
                                                class="px-0 btn btn-lg w-100 btn-info" data-toggle="modal"
                                                style="font-weight: bold" data-target="#newCustomerModal">
                                                RESTORE
                                            </a>
...
const addProduct = async () => {
    ...
    try {
        ...
        if (remark.value !== null || selectCustomer.value !== null) {
            ...
            const response = (await axios.post(route("return.item.store", page_props.order_id), selected_product.value)).data;
...
const getReturnProducts = async () => {
    ...
    try {
        const returnProductData = (await axios.get(route('return.get.products', page_props.order_id))).data;
...
const searchLoyaltyCustomer = async () => {
    ...
    try {
        ...
        loyalty_customer.value.name = truncateText(tableData.name);
...
const saveCustomer = async (customer) => {
    ...
    try {
        ...
        if (customer) {
            const customer_all_details = (await axios.post(route('return.customer', page_props.order_id), customer)).data;
...
const removeCustomer = async () => {
    ...
    try {
        if (returnData.value.remark == null || returnData.value.remark.length <= 0) {
            errorMessage('If you want to remove the customer, add & update the remark first');
        } else {
...
const calculateTotals = async () => {
    ...
    try {
        const calTotal = (await axios.get(route('return.get.total', page_props.order_id))).data;
...
const getReturnOrder = async () => {
    ...
    try {
        const order = (await axios.get(route('return.edit.order', page_props.order_id))).data;
        ...
        select_currency.value = order.currency;
        nextTick(() => {
            loading_bar.value.finish();
        });
    } catch (error) {
...
const saveItem = async () => {
    ...
    try {
        ...
        if (order_status.value == 1) {
            ...
            if (length <= 0 && isDeleteItem == false) {
                ...
                const data = {
                    order_id: page_props.order_id,
                    order_total: total.value,
                    remark: remark.value,
                    currency_id: select_currency_id.value,
                };
                try {
                    await axios.post(route('return.update'), data);
        ...
        if (order_status.value == 1) {
            ...
            if (length <= 0) {
                ...
                try {
                    ...
                    const print = await axios.get(route('return.print', page_props.order_id), { responseType: "blob" });
...
const restoreReturn = async () => {

    nextTick(() => {
        loading_bar.value.start();
    });
    try {
        const response = (await axios.get(route("return.restore", page_props.order_id))).data;
        window.location.href = route("return.view");
        successMessage('Return restored successfully');
